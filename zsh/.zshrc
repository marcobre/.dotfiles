...
#if [[ "${XDG_CACHE_HOME:-$HOME/.cache}/p10k-instant-prompt-${(%):-%n}.zsh" ]]; then
...
# Aliases for nvim-configs & nvims config chooser function
# change to whatever other nvim config you want to use in parallel
...
function fzf-nvim () {
    if $(check_req); then
        ...
        if [ -n "$selected_file" ]; then
            BUFFER="nvim-marcom $selected_file"
            zle accept-line
        fi
        zle reset-prompt
